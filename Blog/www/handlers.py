...
def text2html(text):
    text_2_lines = filter(lambda s: s.strip() != '', text.split('\n'))
    lines = map(
        lambda s: '<p>%s</p>' % (s.replace('&', '&amp;').replace('<', '&lt;')
                                 .replace('>', '&gt;')), text_2_lines)
    return ''.join(lines)
...
def user2cookie(user, max_age):
    ...
    """
    # build cookie string by: id-exprires-sha1
